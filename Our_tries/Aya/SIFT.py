...
class SIFTMatcher:
    ...
    def draw_matches(self, image1, keypoints1, image2, keypoints2, matches):
        ...
        drawn_image = cv2.drawMatches(image1, keypoints_list1, image2, keypoints_list2, dmatches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
        return drawn_image
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        image1 = cv2.imread("D:/ComputerVision/Filtering_and_Edge_Detection_Studio/test-images/12.jpg")
...
# ---------------------------------edited--------------------------------
        ...
        # if len(matches) >= 0.5(height*width):
        #     print("----------------------------Images matched successfully!------------------------")
    ...
    def construct_scale_space(self, image):
        ...
        # base_image = cv2.resize(base_image, (0, 0), fx=2, fy=2, interpolation=cv2.INTER_LINEAR)
        # base_image = image
        ...
        for i in range(0, num_octaves):
            ...
            # base_image = cv2.resize(octaves[i][2], (octaves[i][2].shape[1] // 2, octaves[i][2].shape[0] // 2), interpolation=cv2.INTER_NEAREST)
        # ---------------- show octaves ----------------------------
