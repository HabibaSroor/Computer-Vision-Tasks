...
#---------------------------------------------------------------- Testing functions :-------------------------------------------------------------
img_path = "C:/Study/CV/CV_Tasks/Filtering_and_Edge_Detection_Studio/Our_tries/Hager/images/cow_step_harris.png"
# img_path = "C:/Study/CV/CV_Tasks/Filtering_and_Edge_Detection_Studio/Our_tries/Hager/images/Black & white boxes.jpg"
img = cv2.imread(img_path)
start_time = timeit.default_timer()
# harris_response = apply_harris_operator(img)
harris_response = apply_lamda_minus_operator(img)
corner_indices, edges_indices, flat_indices = get_harris_indices(harris_response)
corner_color = [255, 0, 0]  
edge_color = [0, 255, 0]  
flat_color = [0, 0, 255]  
img_with_corners = map_indices_to_image(img, corner_indices, corner_color)
img_with_edges = map_indices_to_image(img, edges_indices, edge_color)
img_with_flat = map_indices_to_image(img, flat_indices, flat_color)
...
plt.title('Corners')
plt.subplot(1, 3, 2)
plt.imshow(cv2.cvtColor(img_with_edges, cv2.COLOR_BGR2RGB))
plt.title('Edges')
plt.subplot(1, 3, 3)
plt.imshow(cv2.cvtColor(img_with_flat, cv2.COLOR_BGR2RGB))
plt.title('Flat areas')
plt.show()
