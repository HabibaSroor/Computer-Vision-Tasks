...
img_path = "C:/Study/CV/CV_Tasks/Filtering_and_Edge_Detection_Studio/Our_tries/Hager/images/Sudoko.png"
# C:\Study\CV\CV_Tasks\Filtering_and_Edge_Detection_Studio\Our_tries\Hager\images\Sudoko.png
image = cv2.imread(img_path)
if image is None:
    print("Error: Unable to load the image. Please check the file path or integrity of the image.")
    exit()
# Apply my Hough line detection function
result_custom = hough_lines(50, 150, 3, image)

# Apply OpenCV's Hough Line Transform
gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
edges = cv2.Canny(gray, 50, 150, apertureSize=3)
lines = cv2.HoughLines(edges, 1, np.pi / 180, 200)
# Draw lines detected by OpenCV same length of line as i make 
result_opencv = np.copy(image)
