...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        image = cv2.imread("C:/Users/ashra/OneDrive/Documents/Filtering_and_Edge_Detection_Studio/Salma/images.jpeg")
    ...
    def create_initial_contour(self, source):
        ...
        radius = 0.25 * min(source.shape)
        # print(img.siz)
        center_x, center_y = source.shape[0]/2, source.shape[1]/2
        x = int(center_x + radius * np.cos(angles))
        y = int(center_y + radius * np.sin(angles))
        return np.array([x, y]).T

    # from https://github.com/r-CV-Tasks/Computer-Vision-Tasks/blob/main/libs/Contour.py
    def create_square_contour(self, source, num_xpoints=200, num_ypoints=200):
        step = 2
